...
import { ProducerTypeEnum, SignOutTypeEnum, UserRoleEnum } from 'enums/user';
import {
  getAllPermissions,
...
class Auth extends VuexModule implements AppStore.AuthState {
  ...
  public get isFirstUser(): boolean {
    return this.user && this.user.isFirstUser;
  }
  ...
  public get isAdminType(): boolean {
    return this.user && this.user.userType === ProducerTypeEnum.ADMIN;
  }
  ...
  public get hasManagePartnerMenuForProducer(): boolean {
    return (
      this.hasManagePartnerMenu &&
      (this.showAddBroker ||
        this.showAddProcessingFacility ||
        this.hasLogTransport)
    );
  }
  ...
  public get hasReportListMenu(): boolean {
    return this.permissions.includes(
      PermissionActionEnum.SUBMIT_GRIEVANCE_REPORTS,
    );
  }
  ...
  public get hasAssignCAP(): boolean {
    return this.permissions.includes(PermissionActionEnum.ASSIGN_CAP);
  }

  public get hasViewCAP(): boolean {
    return this.permissions.includes(PermissionActionEnum.VIEW_CAP);
  }
  ...
  public get hasLogPurchase(): boolean {
  ...
  public get hasLogTransformation(): boolean {
  ...
  public get hasLogTransport(): boolean {
  ...
  public get showAddBroker(): boolean {
    return this.permissions.some((action) =>
      [
        PermissionActionEnum.ALLOW_PURCHASE_INTERMEDIARIES,
        PermissionActionEnum.ALLOW_SALE_INTERMEDIARIES,
      ].includes(action),
    );
  }

  public get showAddProcessingFacility(): boolean {
    return this.permissions.some((action) =>
      [
        PermissionActionEnum.LOG_SALE,
        PermissionActionEnum.LOG_PURCHASE,
  ...
  public async signIn({
    ...
    try {
      ...
      const isAPIUser = isAPIRole(response.user);
      if (!isAPIUser) {
