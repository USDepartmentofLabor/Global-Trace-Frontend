...
export function supplierOnboard(context: App.RouteContext): void {
  return redirectByPermission({
    context,
    permission: null,
    validator: (): boolean => !producerRegistered(context.user),
  });
}
...
export function isSystemConfiguring(context: App.RouteContext): void {
  return redirectByPermission({
    context,
    permission: null,
    validator: (): boolean => !isCompletedConfiguration(context.user),
  });
}

export function isSystemConfigured(context: App.RouteContext): void {
  return redirectByPermission({
    context,
    permission: null,
    validator: (): boolean => isCompletedConfiguration(context.user),
