...
export default class SAQ extends Vue {
  ...
  get isLastStep(): boolean {
    if (this.currentAnswer && !isEmpty(this.currentAnswer.answerValues)) {
      const response = this.currentQuestion.questionResponses.find(
        ({ id }) =>
          id ===
          get(
            head(this.currentAnswer.answerValues),
  ...
  get isLastQuestion(): boolean {
    if (this.currentAnswer && !isEmpty(this.currentAnswer.answerValues)) {
      ...
      return response && !response.nextQuestionId && !this.isLastGroup;
    }
  ...
  renderPartTitle(): JSX.Element {
    return (
      <Styled.Part>
        {get(this.currentSAQGroup, 'title')[this.currentLocale]}
      </Styled.Part>
    );
  }
