...
import { flatMap, get, isEmpty, maxBy, pick } from 'lodash';
...
export default class SupplierMap extends Mixins(MapMixin) {
  ...
  initNodes() {
    ...
    this.data.forEach((chain, chainIndex) => {
      const chainTop = this.getChainTop(chainIndex);
      chain.forEach(({ suppliers, type }, supplierGroupIndex) => {
        const groupTop = this.getGroupTop(chainIndex, type);
        const isExist = this.isExistInTiers(type);
        if (isExist) {
          const isRoot = supplierGroupIndex === chain.length - 1;
          const { x } = tierNodes.find(({ label }) =>
            label.toLowerCase().split(' | ').includes(type.toLowerCase()),
          );
          suppliers = suppliers.reverse().map((supplier, index) => {
            ...
            supplier.x = x;
            supplier.y =
              (NODE_HEIGHT.MEDIUM + NODE_SPACE_Y) * index + chainTop + groupTop;
            return supplier;
          });
  ...
  getGroupTop(chainIndex: number, type: string): number {
    const chain = this.data[chainIndex];
    let top = 0;
    const tierGroups = this.tiers.find((tierGroup) =>
      flatMap(tierGroup, 'name').includes(type),
    );
    for (const tierGroup of tierGroups) {
      if (tierGroup.name.toLowerCase() !== type.toLowerCase()) {
        const tierSuppliers = chain.find(
          (item) => item.type.toLowerCase() === tierGroup.name.toLowerCase(),
        );
        if (tierSuppliers) {
          top +=
            (NODE_HEIGHT.MEDIUM + NODE_SPACE_Y) *
            tierSuppliers.suppliers.length;
        }
      } else {
        break;
      }
  ...
  getParents(
    ...
    const parents: BrandSupplier.SupplierItem[] = [];
    this.data.forEach((chain: BrandSupplier.SupplierMapGroup[]) => {
      chain.forEach((tierGroup: BrandSupplier.SupplierMapGroup) => {
  ...
  getChainTop(groupIndex: number): number {
    let top = 0;
