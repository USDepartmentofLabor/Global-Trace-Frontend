...
export default class IndicatorManagement extends Vue {
  ...
  private readonly severityOptions: App.DropdownOption[] = [
    {
      name: this.$t('high'),
  ...
  addLaborRisk(): void {
    this.laborRisks.push({
      category: get(this.indicatorSelected, 'category'),
    ...
    requestModule.resetSubIndicators();
  }
  ...
  getSubIndicators() {
    if (this.indicatorSelected) {
      ...
      requestModule.getSubIndicators({
        parentIds: [this.indicatorSelected.id as string],
        callback: {
    ...
    return (
      <Styled.FilterContainer>
        <Styled.FilterWrapper
          sortType={this.filters.category}
          vOn:click={() => this.changeSortFilter('category')}
        >
      ...
      <fragment>
        {this.laborRisks.map((item, index) => (
          <Styled.List key={index}>
            <Styled.ListItem>{get(item, 'category.name')}</Styled.ListItem>
