...
export default class FileUpload extends Vue {
  @PropSync('values', { default: [] }) defaultValues: App.UploadFilesResponse[];
  @Prop({
    default: 'primary',
  ...
  changeDefaultFiles: (files: App.UploadFilesResponse[]) => void;

  private selectedFiles: App.SelectedFile[] = [];

  get isSelectedFile(): boolean {
    return !isEmpty(this.selectedFiles) || !isEmpty(this.defaultValues);
  }
  ...
  removeDefaultFile(index: number) {
    this.defaultValues.splice(index, 1);
    this.changeDefaultFiles(this.defaultValues);
  }
  ...
  renderDefaultFiles(): JSX.Element {
    return (
      <fragment>
        {this.defaultValues.map(({ fileName }, index) => (
          <Styled.FileBox>
            <Styled.FileItem variant={this.variant}>
              <font-icon
                name={this.getFileIcon(fileName)}
                color="envy"
                size="18"
              />
              <Styled.FileName>{fileName}</Styled.FileName>
              <Styled.ButtonRemove
                vOn:click={() => this.removeDefaultFile(index)}
              >
