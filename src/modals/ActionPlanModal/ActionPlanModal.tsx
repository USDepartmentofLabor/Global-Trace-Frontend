...
export default class ActionPlanModal extends Vue {
  ...
  created() {
    if (this.capId) {
      ...
      this.issuedDetail = this.facility.name;
    }
  }
  ...
  setIssueByCAP(): void {
    ...
    let { name } = this.facility;
    const createdAt = get(this.CAPDetail, [
      'indicatorRiskData',
      'subIndicatorRiskData',
      'cap',
      'createdAt',
    ...
    const isAssignee = get(userFacility, 'id') !== createdFacility.id;
    if (isAssignee) {
