...
export default class Indicator extends Vue {
  ...
  initCheckboxGroup(): void {
    const values = this.defaultValues.includes(this.indicator.id)
      ? filter(this.defaultValues, (id: string) =>
          some(
            this.options,
            (option: App.DropdownOption) => option.value === id,
          ),
        )
      : [];
  ...
  changeCheckboxGroup(values: string[]) {
    this.setValues(values);
    this.parentValue = !isEmpty(values);
    this.onChangeCheckboxGroup();
  }
