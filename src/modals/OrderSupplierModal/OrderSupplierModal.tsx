...
export default class OrderSupplierModal extends Vue {
  ...
  set purchasedAt(value: Date) {
    this.$formulate.setValues(this.formName, {
      ...this.formData,
      purchasedAt: value ? dateToISOString(value) : null,
    });
  }
  ...
  onChangeDateTime(purchaseDate: string): void {
    this.purchasedAt = purchaseDate ? moment(purchaseDate).toDate() : null;
  }
  ...
  render(): JSX.Element {
    return (
      <modal-layout title={this.modalTitle} closeModal={this.closeModal}>
        <Styled.Content>
          <formulate-form
