...
export default class FacilityGroupInformationModal extends Vue {
  ...
  @Prop({ required: true }) setCurrentSupplierId: (id: string) => void;
  ...
  async onImportFile(): Promise<void> {
    try {
      ...
      this.getFacility();
      const supplierId = get(this.facility, ['farms', 0, 'id']);
      this.setCurrentSupplierId(supplierId);
    } catch (error) {
      handleError(error as App.ResponseError);
    }
  }
  ...
  renderFacility(facility: Auth.Facility): JSX.Element {
    return (
      <fragment>
        ...
        <Styled.Field>{this.$t('street_address')}</Styled.Field>
        <Styled.Value>
          {get(facility, 'additionalInformation.streetAddress')}
        </Styled.Value>
